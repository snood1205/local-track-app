...
RSpec.describe 'Tracks' do
  ...
  describe 'GET /tracks/info' do
    ...
    describe 'Track information' do
      ...
      it { expect(parsed_track['lengthUnit']).to eq(track.length_unit) }
      it { expect(parsed_track['humanReadableLength']).to eq("#{track.length} #{track.length_unit}") }
    ...
    describe 'address information' do
      ...
      it { expect(parsed_address['streetLine1']).to eq(address.street_line1) }
      it { expect(parsed_address['streetLine2']).to eq(address.street_line2) }
      it { expect(parsed_address['city']).to eq(address.city) }
      it { expect(parsed_address['region']).to eq(address.region) }
      it { expect(parsed_address['postalCode']).to eq(address.postal_code) }
