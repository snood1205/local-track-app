...
RSpec.describe 'Vendors' do
  ...
  describe 'GET /index' do
    ...
    %w[id name category].each do |attribute|
  ...
  describe 'GET /grouped' do
    ...
    it { expect(response.parsed_body['Food & Drinks'][0]['id']).to eq(beverage_vendor.id) }
    it { expect(response.parsed_body['Food & Drinks'][0]['name']).to eq(beverage_vendor.name) }
    it { expect(response.parsed_body['Food & Drinks'][1]['id']).to eq(food_vendor.id) }
    ...
    it { expect(response.parsed_body['Clothing'][0]['id']).to eq(merch_vendor.id) }
